...
#import "LCQTypes.h"
#endif
#import "LCQNetworkTrace.h"

@class LCQExecutionTrace;

NS_ASSUME_NONNULL_BEGIN

NS_SWIFT_NAME(APM)
/// Luciq APM
@interface LCQAPM : NSObject

/// Disables/Enables APM.
///
/// Defaults to true if APM is included in your Luciq account's plan.
...
+ (LCQExecutionTrace *_Nullable)startExecutionTraceWithName:(NSString *)name __attribute__((unavailable("Please migrate to the App Flows APIs: APM.startFlow, APM.endFlow, and APM.setAttributeForFlow.")));
...
+ (NSString* _Nullable)addNetworkTraceAttributesForURLMatchingPredicate:(NSPredicate* _Nullable)urlPredicate
                                                           ...
                                                           usingHandler:(NSDictionary<NSString *, NSString *> * _Nullable(^)(LCQNetworkTrace *networkTrace))handler;
