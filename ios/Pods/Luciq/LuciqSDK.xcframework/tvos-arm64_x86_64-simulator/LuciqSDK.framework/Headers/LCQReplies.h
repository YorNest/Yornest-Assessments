...
@interface LCQReplies : NSObject

/**
 @brief Acts as a master switch for the In-app Replies.
 
 @discussion It's enabled by default. When disabled, the user can’t reach the chats history from the SDK. The chats list button is removed from Luciq Prompt Options. In addition, when disabled the in-app notification as well as the push notifications are disabled. And, +show won’t have an effect.
 */
...
/**
 @brief Call this method and pass the notification's userInfo dictionary to allow Luciq to handle its remote notifications.
 
 @discussion Luciq will check if notification is from Luciq's servers and only handle it if it is.
...
/**
 @brief Use this method to set Apple Push Notification token to enable receiving Luciq push notifications.
