...
#import "LCQSurvey.h"
#import "LCQSurveyFinishedState.h"

NS_ASSUME_NONNULL_BEGIN

NS_SWIFT_NAME(Surveys)
@interface LCQSurveys : NSObject
...
/**
 ...
 @discussion If you disable surveys auto showing on the SDK but still have active surveys on your Luciq dashboard, those surveys are still going to be sent to the device, but are not going to be shown automatically.
 ...
 */
+ (void)availableSurveysWithCompletionHandler:(void (^)(NSArray<LCQSurvey *> * validSurveys))completionHandler;
 ...
 *
 *      * `LCQSurveyFinishedState state` - An enum describing the state of the survey on finishing, whether it was dismissed at start
 ...
 */
@property (class, atomic, strong, nullable) void(^didFinishSurveyHandler)(LCQSurveyFinishedState state, NSDictionary *info, NSString *identifier);
